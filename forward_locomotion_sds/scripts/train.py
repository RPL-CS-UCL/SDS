...
def train_mc(iterations, command_config, reward_config, dr_config, sds_target_velocity=None,
    ...
    from forward_locomotion_sds.go1_gym.envs.base.legged_robot_config import Cfg, set_seed
    ...
    from forward_locomotion_sds.go1_gym_learn.ppo import Runner
    from forward_locomotion_sds.go1_gym.envs.wrappers.history_wrapper import HistoryWrapper
    from forward_locomotion_sds.go1_gym_learn.ppo.actor_critic import AC_Args
    from forward_locomotion_sds.go1_gym_learn.ppo.ppo import PPO_Args
    from forward_locomotion_sds.go1_gym_learn.ppo import RunnerArgs
    ...
    if reward_config == "original":
        ...
        assert sds_target_velocity is None
    elif reward_config == "sds":
        Cfg.rewards = Cfg.rewards_sds
        if sds_target_velocity is not None:
            Cfg.rewards.target_velocity = sds_target_velocity
    else:
        raise NotImplementedError
    ...
    config_go1(Cfg)
    print(f"Training on {Cfg.env.num_envs} environments")
    ...
    wandb.init(
        project="forward_locomotion_sds",
        entity="upenn-pal",
...
if __name__ == '__main__':
    ...
    from ml_logger import logger
    from forward_locomotion_sds.go1_gym import MINI_GYM_ROOT_DIR

    stem = Path(__file__).stem
    logger.configure(logger.utcnow(f'forward_locomotion_sds/%Y-%m-%d/{stem}/%H%M%S.%f'),
    ...
    parser.add_argument("--reward-config", type=str, required=True, choices=["original", "sds"])
    parser.add_argument("--dr-config", type=str, required=True, choices=["original", "sds", "off"])

    parser.add_argument("--sds-target-velocity", type=float)
