...
from forward_locomotion_sds.go1_gym.envs.base.base_task import BaseTask
from forward_locomotion_sds.go1_gym.utils.math_utils import quat_apply_yaw, wrap_to_pi, get_scale_shift
from forward_locomotion_sds.go1_gym.utils.terrain import Terrain
...
class LeggedRobot(BaseTask):
    ...
    def check_termination(self):
        ...
        self.num_resets += self.reset_buf.sum()
    ...
    def _prepare_reward_function(self):
        ...
        from forward_locomotion_sds.go1_gym.rewards.original_reward import OriginalReward
        from forward_locomotion_sds.go1_gym.rewards.sds_reward import SDSReward
        if reward_container_name == "OriginalReward":
            self.reward_container = OriginalReward(self)
        elif reward_container_name == "SDSReward":
            self.reward_container = SDSReward(self)
        else:
    ...
    def get_camera_position(self,target_x,target_y,target_z):
        # return gymapi.Vec3(target_x, target_y - 1.0, target_z + 1.0)
        if self.cfg.env.camera_mode == "corner":
            return gymapi.Vec3(target_x, target_y - 0.7, target_z+0.5)
        elif self.cfg.env.camera_mode == "sideways":
            return gymapi.Vec3(target_x, target_y - 1, target_z)
        

    def render(self, mode="rgb_array"):
        assert mode == "rgb_array"
        bx, by, bz = self.root_states[0, 0], self.root_states[0, 1], self.root_states[0, 2]
        self.gym.set_camera_location(self.rendering_camera, self.envs[0], self.get_camera_position(bx,by,bz),
